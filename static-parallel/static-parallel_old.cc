...
namespace Step40
{
      ...
      if (cell->is_locally_owned())
        {
          ...
          for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
            {
                fe_values.get_function_values(locally_relevant_solution,present_values);
               std::cout << "cellnum  " <<cellnum<<" present_values " << present_values[0] <<std::endl;
              const double
              rhs_value
  ...
  void LaplaceProblem<dim>::output_results (const unsigned int cycle) const
  {
    ...
    Vector<float> subdomain (triangulation.n_active_cells());
      std::cout << "subdomain " << subdomain.size() <<std::endl;
