...
%
  ...
  %tau(1,1:element_num) = tauc(1:element_num)';
  ...
  p = zeros ( 1, node_num );
  p(1:node_num) = uvp(1:node_num,3)';
%
%  Write the data.
%
  vtk_puv_write ( output_unit, title, node_num, element_num, ...
    element_order2, xyz, element_node, uvw,p);
...
function vtk_puv_write ( output_unit, title, node_num, element_num, ...
  element_order, xyz, element_node, uvw,p)
...
%
  ...
  fprintf ( output_unit, 'SCALARS p double\n' );
  fprintf ( output_unit, 'LOOKUP_TABLE default\n' ); 
  for node = 1 : node_num
    fprintf ( output_unit, '  %2.16f\n', p(1,node) );
  end
