function [u2]=mapuv(IBC2,BCval2,p2,IEN_all2,p_All2,vertexData2,p1,IEN_all1,p_All1,vertexData1,variable)
...
quadsf = QuadShapeFunc(qPoints,p2);
[ShapeFunc2, ~] = quadsf.QuadShapeFuncTable();

quadsf = QuadShapeFunc(qPoints,p1);
...
%value
disp('add bc')
[row2,column2,value,rhs] = StrongBC(TotalDOF2,IBC2,BCval2,row2,column2,value,rhs);
...
function [elementK,elementF] = eleMapping(sizeN,xCord,yCord,qPoints,ShapeFunc1,ShapeFunc2,uv1)
%sizeN = length(IENall2);
sizeM = sizeN/2;
%elementK = zeros(sizeN, sizeN);
%elementF = zeros(sizeN,1);
elementKu = zeros(sizeM, sizeM);
elementFu = zeros(sizeM,1);
elementKv = zeros(sizeM, sizeM);
elementFv = zeros(sizeM,1);
...
for k = 1:nP
    ...
    vquad = ShapeFunc1(:,k)'*v1;
    
    elementFu = elementFu+ShapeFunc2(:,k)*Jw*uquad;
    elementFv = elementFv+ShapeFunc2(:,k)*Jw*vquad;
    NbGlobal = ShapeFunc2(:,k);
    
    %lhs
    elementKu = elementKu + NbGlobal*NbGlobal'*Jw;
    elementKv = elementKv + NbGlobal*NbGlobal'*Jw;
end
elementK = [elementKu,zeros(sizeM,sizeM);zeros(sizeM,sizeM),elementKv];
elementF = [elementFu;elementFv];

%elementK\elementF
end

function [row,column,value,fGlobal] = StrongBC(TotalDOF,IBC,BCval,...
